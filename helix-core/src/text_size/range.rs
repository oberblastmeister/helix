...
impl TextRange {
    ...
    /// The opposite of intersection. Returns two new ranges that do not intersect.
    /// If the ranges touch but do not overlap, the same ranges will be returned in increasing order.
    /// If the ranges are the same, `None` will be returned.
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    ///
    /// assert_eq!(
    ///     TextRange::difference(
    ///         TextRange::new(0, 10),
    ///         TextRange::new(5, 15),
    ///     ),
    ///     Some((TextRange::new(0, 5), TextRange::new(10, 15))),
    /// );
    ///
    /// assert_eq!(
    ///     TextRange::difference(
    ...
    ///         TextRange::new(0, 10),
    ///     ),
    ///     Some((TextRange::new(10, 15), TextRange::new(0, 5))),
    /// );
    ///
    /// assert_eq!(
    ///     TextRange::difference(
    ...
    ///         TextRange::new(10, 15),
    ///     ),
    ///     Some((TextRange::new(5, 10), TextRange::new(10, 15))),
    /// );
    ///
    /// assert_eq!(
    ///     TextRange::difference(
    ...
    ///         TextRange::new(5, 10),
    ///     ),
    ///     Some((TextRange::new(10, 15), TextRange::new(5, 10))),
    /// );
    ///
    /// assert_eq!(
    ///     TextRange::difference(
    ...
    ///         TextRange::new(0, 1),
    ///     ),
    ///     Some((TextRange::new(3, 5), TextRange::new(0, 1))),
    /// );
    /// ```
    pub fn difference(self, other: TextRange) -> Option<(TextRange, TextRange)> {
        if self == other {
            return None;
        }
        Some(if let Some(intersect) = self.intersect(other) {
            let res = (
                TextRange::new(cmp::min(self.start(), other.start()), intersect.start),
                TextRange::new(intersect.end, cmp::max(self.end(), other.end())),
            );
            if res.0.start() == self.start() {
                res
            } else {
                (res.1, res.0)
            }
